/* * kspman v0.3
 * Build: gcc -o kspman.exe kspman.c -mwindows -lcomctl32 -lwinhttp -lole32 -loleaut32 -luuid -lshell32 -lshlwapi -ladvapi32 -s
 * License: GNU General Public License v3.0 (GPLv3)
 * Repository: https://github.com/FacePafes/kspman
 */
...
DEP_DATA* CreateDepData(int initial_capacity) {
    DEP_DATA* data = (DEP_DATA*)malloc(sizeof(DEP_DATA));
    if (data) {
        data->count = 0;
        data->capacity = initial_capacity;
        data->links = (char**)malloc(sizeof(char*) * data->capacity);
        data->names = (char**)malloc(sizeof(char*) * data->capacity);
        if (!data->links || !data->names) {
            free(data->links);
            free(data->names);
            free(data);
            return NULL;
...
void ApplyTheme(const char* json) {
    ...
    if (json && strlen(json) > 0) {
        if ((p = strstr(json, "\"Background\": \""))) bg_color = HexToCol(p + strlen("\"Background\": \""));
        if ((p = strstr(json, "\"Text\": \""))) text_color = HexToCol(p + strlen("\"Text\": \""));
...
void FreeDepData(DEP_DATA* data) {
...
void AddDependency(DEP_DATA* data, const char* link, const char* name) {
    if (data->count >= data->capacity) {
        int new_capacity = data->capacity * 2;
        
        char** new_links = (char**)realloc(data->links, sizeof(char*) * new_capacity);
        char** new_names = (char**)realloc(data->names, sizeof(char*) * new_capacity);

        if (!new_links || !new_names) {
            if (new_links) data->links = new_links;
            if (new_names) data->names = new_names;
            return;
        }
    ...
    data->links[data->count] = _strdup(link);
    data->names[data->count] = _strdup(name);
...
void LoadConfig() {
    ...
    if (!f) {
            ...
            RegOpenKeyExA(HKEY_LOCAL_MACHINE, subkey, 0, KEY_READ | KEY_WOW64_32KEY, &hKey) == ERROR_SUCCESS) 
        {
            ...
            if (RegQueryValueExA(hKey, "InstallLocation", NULL, NULL, (LPBYTE)steamPath, &len) == ERROR_SUCCESS) {
                _snprintf_s(ksp_path, sizeof(ksp_path), _TRUNCATE, "%s\\GameData", steamPath);
            }
            RegCloseKey(hKey);
        }
        g_Opacity = 255; g_FontSize = 18; g_ListViewGrid = TRUE;
    ...
    if ((p = strstr(buf, "\"Path\": \""))) {
        ...
        if (end) {
            ...
            /* unescape \\ -> \ as written by SaveConfig/EnsureConfigExists */
            int di = 0;
            for (int si = 0; si < len && di < MAX_PATH - 1; si++) {
                if (start[si] == '\\' && si + 1 < len && start[si + 1] == '\\') {
                    ksp_path[di++] = '\\';
                    si++;
                } else {
                    ksp_path[di++] = start[si];
                }
            }
            ksp_path[di] = '\0';
        }
    ...
    char *key;
    if ((key = strstr(buf, "\"font_size\":"))) sscanf(strchr(key, ':') + 1, "%d", &g_FontSize);
    if ((key = strstr(buf, "\"opacity\":"))) sscanf(strchr(key, ':') + 1, "%d", &g_Opacity);
    if ((key = strstr(buf, "\"list_view_grid\":"))) {
        char *val = strpbrk(key, "tf");
        if (val) g_ListViewGrid = (strncmp(val, "true", 4) == 0);
    }
    if ((key = strstr(buf, "\"Width1\":"))) sscanf(strchr(key, ':') + 1, "%d", &col_widths[0]);
    if ((key = strstr(buf, "\"Width2\":"))) sscanf(strchr(key, ':') + 1, "%d", &col_widths[1]);
    if ((key = strstr(buf, "\"Width3\":"))) sscanf(strchr(key, ':') + 1, "%d", &col_widths[2]);
...
void ScanGameData() {
    if (!ksp_path[0]) {
        MessageBoxA(NULL, "KSP Path is not set.", "Error", MB_ICONERROR);
        return;
    }

    if (!PathFileExistsA(ksp_path)) {
        int msgBoxResult = MessageBoxA(NULL, 
            "The configured KSP path does not exist. do you want the code to rescan the registry for the game?", 
            "Path Not Found", 
            MB_ICONERROR | MB_YESNO);

        if (msgBoxResult == IDYES) {
            char temp_path[MAX_PATH];
            GetKSPPathFromRegistry(temp_path); 
            
            if (PathFileExistsA(temp_path)) {
                strcpy(ksp_path, temp_path);
                SaveConfig();
            } else {
                MessageBoxA(NULL, "Could not find valid KSP path in Registry.", "Error", MB_ICONERROR);
                return;
            }
        } else {
            return;
    ...
    char filter[256]; 
    GetWindowTextA(hSearch, filter, 256);
    
    char s[MAX_PATH]; 
    _snprintf_s(s, MAX_PATH, _TRUNCATE, "%s\\*", ksp_path);
    
    WIN32_FIND_DATAA f; 
    HANDLE h = FindFirstFileA(s, &f);
    
    if (h == INVALID_HANDLE_VALUE) {
        MessageBoxA(NULL, "Failed to access the game directory.", "Access Error", MB_ICONERROR);
        return;
    }
    ...
    do {
        if ((f.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && f.cFileName[0] != '.') {
            ...
            lv.pszText = f.cFileName;
            int idx = ListView_InsertItem(hListView, &lv);
            
            char m[MAX_PATH]; 
            _snprintf_s(m, MAX_PATH, _TRUNCATE, "%s\\%s\\kspman_manifest.txt", ksp_path, f.cFileName);
            ListView_SetItemText(hListView, idx, 1, PathFileExistsA(m) ? "Managed" : "Loose");
            
            char modPath[MAX_PATH];
            _snprintf_s(modPath, MAX_PATH, _TRUNCATE, "%s\\%s", ksp_path, f.cFileName);
            char vStr[32];
...
void ToggleMod(const char* name, BOOL enable) {
    ...
    if (enable) {
        snprintf(oldP, sizeof(oldP), "%s\\%s.disabled", ksp_path, name);
        snprintf(newP, sizeof(newP), "%s\\%s", ksp_path, name);
    } else {
        snprintf(oldP, sizeof(oldP), "%s\\%s", ksp_path, name);
        snprintf(newP, sizeof(newP), "%s\\%s.disabled", ksp_path, name);
    }

    if (GetFileAttributesA(newP) != INVALID_FILE_ATTRIBUTES) {
        char msg[512];
        snprintf(msg, sizeof(msg), "Target path already exists:\n%s\n\nDelete the existing folder and proceed?", newP);
        
        if (MessageBoxA(hMainWnd, msg, "Conflict", MB_YESNO | MB_ICONWARNING) == IDYES) {
            char newP_dn[MAX_PATH + 2] = {0};
            strncpy(newP_dn, newP, MAX_PATH - 1);
            /* SHFileOperationA requires double-null termination; buffer is zero-
               initialised so newP_dn[len] and newP_dn[len+1] are already '\0'. */
            SHFILEOPSTRUCTA s = {0};
            s.hwnd = hMainWnd;
            s.wFunc = FO_DELETE;
            s.pFrom = newP_dn;
...
retry_move:
    if (!MoveFileA(oldP, newP)) {
        ...
        char errBuf[512];
        snprintf(errBuf, sizeof(errBuf), "failed to move folder (Error %lu).\nis the game or a file open?\n\nPath: %s", err, oldP);
...
void CheckAndPromptDeps(const char* stagePath, HWND parent) {
    ...
    swprintf_s(wDepFile, MAX_PATH, L"%ls\\dependencies.txt", wStagePath);

    if (GetFileAttributesW(wDepFile) == INVALID_FILE_ATTRIBUTES) {
        WCHAR wSearch[MAX_PATH];
        swprintf_s(wSearch, MAX_PATH, L"%ls\\*\\dependencies.txt", wStagePath);
        WIN32_FIND_DATAW fd;
        HANDLE h = FindFirstFileW(wSearch, &fd);
        if (h != INVALID_HANDLE_VALUE) {
            swprintf_s(wDepFile, MAX_PATH, L"%ls\\%ls\\dependencies.txt", wStagePath, fd.cFileName);
            FindClose(h);
        }
    }
    ...
    DEP_DATA* data = CreateDepData(10); 
    if (!data) return;
    ...
    if (f) {
        ...
        while (fgets(line, sizeof(line), f)) {
            ...
            if (sscanf_s(line, "\"%[^\"]\" \"%[^\"]\"", link, (unsigned)_countof(link), ver, (unsigned)_countof(ver)) >= 1) {
                char display[2200];
                _snprintf_s(display, sizeof(display), _TRUNCATE, "%s (v%s)", link, ver);
                
                AddDependency(data, link, display);
            }
        }
        fclose(f);
    }
...
void InstallFromURL(HWND hwnd) {
    ...
    if (!hRequest || !WinHttpSendRequest(hRequest, WINHTTP_NO_ADDITIONAL_HEADERS, 0, WINHTTP_NO_REQUEST_DATA, 0, 0, 0) || !WinHttpReceiveResponse(hRequest, NULL)) goto cleanup;
...
void CopyRecursiveAndLog_Internal(const char* src, const char* dst, const char* manifestPath) {
    ...
    do {
        ...
        snprintf(sPath, sizeof(sPath), "%s\\%s", src, f.cFileName);
        snprintf(dPath, sizeof(dPath), "%s\\%s", dst, f.cFileName);
...
void CopyRecursiveAndLog(const char* src, const char* dst, const char* manifest) {
    ...
    CopyRecursiveAndLog_Internal(src, dst, manifest);
}
...
void GetModVersion(const char* folderPath, char* outVersion) {
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
        ...
        if (f) {
            ...
            BOOL foundMaj = FALSE, foundMin = FALSE, foundPat = FALSE;
            while (fgets(line, sizeof(line), f)) {
                if (strstr(line, "\"MAJOR\"")) { sscanf(strstr(line, ":"), ": %d", &maj); foundMaj = TRUE; }
                if (strstr(line, "\"MINOR\"")) { sscanf(strstr(line, ":"), ": %d", &min); foundMin = TRUE; }
                if (strstr(line, "\"PATCH\"")) { sscanf(strstr(line, ":"), ": %d", &pat); foundPat = TRUE; }
            }
            if (foundMaj && foundMin && foundPat) sprintf(outVersion, "%d.%d.%d", maj, min, pat);
            fclose(f);
        }
        FindClose(hFind);
    }
...
void GetKSPPathFromRegistry(char* outPath) {
        ...
        RegOpenKeyExA(HKEY_LOCAL_MACHINE, subkey, 0, KEY_READ | KEY_WOW64_32KEY, &hKey) == ERROR_SUCCESS) 
    {
        if (RegQueryValueExA(hKey, "InstallLocation", NULL, NULL, (LPBYTE)steamPath, &len) == ERROR_SUCCESS) {
            RegCloseKey(hKey);
            _snprintf_s(outPath, MAX_PATH, _TRUNCATE, "%s\\GameData", steamPath);
            return;
        }
        RegCloseKey(hKey);
    }
...
unsigned __int64 GetLocalFileSize(const char* filename) {
...
LRESULT CALLBACK StatsProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    if (msg == WM_CREATE) {
        ...
        unsigned __int64 cfgSize = GetLocalFileSize("config.json");
        ...
        case WM_CREATE: {
            ...
            if (cs && cs->lpCreateParams) {
                ...
                if (f) {
                    ...
                    if (b) {
                        fread(b, 1, sz, f); b[sz] = 0;
                        SetWindowTextA(hEdit, b);
                        free(b);
                    }
                    fclose(f);
                } else { SetWindowTextA(hEdit, "Error: manifest file missing."); }
            }
            break;
        }
        case WM_COMMAND:
            if (LOWORD(wp) == ID_BTN_COPY_LOG) {
                int len = GetWindowTextLengthA(hEdit);
                char *b = (char*)malloc(len + 1);
                if (b) {
                    GetWindowTextA(hEdit, b, len + 1);
...
LRESULT CALLBACK SettingsProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    if (msg == WM_CREATE) {
        ...
        if (f) {
            ...
            if (fbuf) { fread(fbuf, 1, fsz, f); fbuf[fsz] = '\0'; SetWindowTextA(hEdit, fbuf); free(fbuf); }
            fclose(f);
        }
    } else if (msg == WM_COMMAND && LOWORD(wp) == ID_BTN_SAVE_CFG) {
        int tlen = GetWindowTextLengthA(hEdit);
        char *buf = (char*)malloc(tlen + 1);
        if (buf) {
            GetWindowTextA(hEdit, buf, tlen + 1);
            FILE *f = fopen("config.json", "w"); if (f) { fputs(buf, f); fclose(f); }
            ApplyTheme(buf); free(buf);
        }
        DestroyWindow(hwnd);
...
LRESULT CALLBACK UninstallProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    switch (msg) {
        ...
        case WM_COMMAND: {
            if (LOWORD(wp) == 1 && ud) {
                char bP[MAX_PATH];
                _snprintf_s(bP, sizeof(bP), _TRUNCATE, "%s\\uninstall_tmp.bat", ksp_path);
                FILE* f = fopen(bP, "w");
                if (f) {
                    ...
                    while (scan && *scan) {
                        ...
                        if (len > 0 && len < MAX_PATH) {
                            ...
                            if (strchr(line, '\"')) {
                                ...
                                remove(bP);
                                MessageBoxA(hwnd, 
                                    "Error: This file' name contains an invalid character that prevents safe uninstalling. "
                                    "to prevent system errors (like poisoning), please do rename the file/folder to avoid using "
                                    "quotes (\") or special command characters. - Face", 
                                    "Alert", MB_ICONERROR);                                return 0;
                            }
        ...
        case WM_COMMAND:
            switch(LOWORD(wParam)) {
                ...
                case ID_MENU_DEV_DEPS: {
                    ...
                    _snprintf_s(depPath, sizeof(depPath), _TRUNCATE, "%s\\%s\\dependencies.txt", ksp_path, modFolderName);
                    ...
                    if (f) {
                        ...
                        char msg[512];
                        _snprintf_s(msg, sizeof(msg), _TRUNCATE, "dependencies.txt created in %s!", modFolderName);
                        ...
                        if (f) {
                            ...
                            if (sz >= 0) {
                                ...
                                if (ud->fileListBuffer) {
                                    size_t bytesRead = fread(ud->fileListBuffer, 1, sz, f);
                                    ud->fileListBuffer[bytesRead] = '\0';
                                }
                            }
                            fclose(f);
                        } else {
...
int WINAPI WinMain(HINSTANCE h, HINSTANCE hp, LPSTR l, int n) {
    ...
    if (!PathFileExistsA(ksp_path)) {
        GetKSPPathFromRegistry(ksp_path);
        
        if (!PathFileExistsA(ksp_path)) {
            MessageBoxA(NULL, "KSP path not found. Please check your config.json.", "Critical Error", MB_ICONERROR);
            return 0; 
        }
        SaveConfig();
    }
